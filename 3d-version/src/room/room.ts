...
import SkillViewInteractionSequence, {
  SkillTransform,
} from "../interaction/skillViewInteractionSequence";
...
const setupWalls = (
  ...
  const leftWallPhysics = createWallPhysics(
    0,
    -10,
    5,
    0,
  ...
  const rightWallPhysics = createWallPhysics(
    0,
    10,
    5,
    0,
  ...
  const backWallPhysics = createWallPhysics(
    0,
    0,
    5,
    -10,
  ...
  const frontWallPhysics = createWallPhysics(
    0,
    0,
    5,
    10,
...
const setupFloor = (
  ...
  const floorPhysics = createFloorPhysics(
    0,
    0,
    0,
    0,
...
const setupMirrors = (scene: THREE.Scene) => {
  const longMirror = createMirror(
    18,
    6,
    new THREE.Vector3(9.999, 3, 0),
...
const setupGymEquipment = async (
  scene: THREE.Scene,
  world: CANNON.World,
  loader: GLTFLoader,
) => {
  // Load dumbbell rack
  const dumbellRack = await loadGLTFModel(
    "assets/models/gym_assets/dumbbell_rack/scene.gltf",
    loader,
  );
  const rack1 = createObjectFromGLTF(dumbellRack, -9, 0, -7, 0, 0, 0, 1, 1, 1);
  scene.add(rack1);
  const rack2 = createObjectFromGLTF(
    dumbellRack,
    -9,
    0,
    -1.4,
  ...
  // load flat bench
  const flatBench = await loadGLTFModel(
    "assets/models/gym_assets/flat_seat/scene.gltf",
    loader,
  );
  const bench = createObjectFromGLTF(
    flatBench,
    0,
    0,
    -5,
  ...
  // load incline bench
  const inclineBench = await loadGLTFModel(
    "assets/models/gym_assets/incline_seat/scene.gltf",
    loader,
  );
  const inclineBenchObject = createObjectFromGLTF(
    inclineBench,
    6.5,
    0,
    -2,
  ...
  // load mattress
  const mattress = await loadGLTFModel(
    "assets/models/gym_assets/mattress/scene.gltf",
    loader,
  );
  const mattressObject = createObjectFromGLTF(
    mattress,
    0,
    0,
    0,
  ...
  // load barbell
  const barbell = await loadGLTFModel(
    "assets/models/gym_assets/barbell/scene.gltf",
    loader,
  );
  const barbellObject = createObjectFromGLTF(
    barbell,
    7,
    0,
    5,
  ...
  const barbellWeights = await loadGLTFModel(
    "assets/models/gym_assets/barbell_weights/scene.gltf",
    loader,
  );
  const barbellWeightsObject = createObjectFromGLTF(
    barbellWeights,
    9,
    0,
    8,
  ...
  //import squat rack
  const squatRack = await loadGLTFModel(
    "assets/models/squat_rack/scene.gltf",
    loader,
  );
  const squatRackObject = createObjectFromGLTF(
    squatRack,
    -9,
    0,
    5,
  ...
  const allObjects = await loadGLTFModel(
    "assets/models/gym_equipment/scene.gltf",
    loader,
  );
  try {
    allObjects.scene.traverse((child) => {
      if (child instanceof THREE.Object3D && child.name === "Bench_press_0") {
        ...
        scene.add(child);
        const barbellPressPhy = createObjectPhysics(
          0,
          6.5,
          1,
          -7,
...
const setupInteractableAreas = async (
  scene: THREE.Scene,
  world: CANNON.World,
  hud: HUD,
  loader: GLTFLoader,
  ...
  const dumbbell = await loadGLTFModel(
    "assets/models/gym_assets/dumbbell/scene.gltf",
    loader,
  );
  ...
  const rackInteraction = rackInteractionBuilder
    ...
    .setCharacterTransform(
      new THREE.Vector3(-5.4, 0, -3.89),
      new THREE.Quaternion(-0, -0.71, -0, 0.71),
    )
    .setDestCameraTransform(
      new THREE.Vector3(-5.12, 3.01, 0.56),
      new THREE.Quaternion(0.05, -0.023, 0.01, 0.99),
    )
  ...
  const rackInteraction3 = rackInteraction3Builder
    ...
    .setDestCameraTransform(
      new THREE.Vector3(-3.36, 3, -4.02),
      new THREE.Quaternion(0.04, 0.7, -0.04, 0.71),
    )
  ...
  const barbellInteraction = barbellInteractionBuilder
    ...
    .setCharacterTransform(
      ...
      new THREE.Quaternion(0, 0.7, 0, 0.72),
    )
    .setDestCameraTransform(
      new THREE.Vector3(-0.57, 3.84, 2.09),
      new THREE.Quaternion(0.11, 0.84, 0.19, -0.49),
    )
  ...
  const barbellInteraction2 = barbellInteractionBuilder2
    ...
    .setAnimations([
      {
        animName: FLEX,
        displayTextList: PROJECT_EXPERIENCE,
        displayTextDur: 8,
      },
    ])
    .setCharacterTransform(
  ...
  const interactable2 = await InteractableArea.create(
    barbellInteractionPos,
    barbellInteractionRot,
    3,
    "Experience",
    barbellTextPos,
    barbellTextRot,
  ...
  const squatRackInteraction = squatRackInteractionBuilder
    ...
    .setCharacterTransform(
      new THREE.Vector3(-4.73, 0, 5.1),
      new THREE.Quaternion(0, -0.7, 0, 0.71),
    )
    .setDestCameraTransform(
      new THREE.Vector3(-1.63, 4.12, -0.55),
      new THREE.Quaternion(-0.06, 0.94, 0.24, 0.25),
    )
  ...
  const interactable3 = await InteractableArea.create(
    squatRackInteractionPos,
    squatRackInteractionRot,
    3.5,
    "Education",
    squatRackTextPos,
    squatRackTextRot,
  ...
  const warmupInteractable = await InteractableArea.create(
    warmupInteractionPos,
    warmupInteractionRot,
    2,
    "Warm Up",
    warmupTextPos,
    warmupTextRot,
  ...
  return [interactable, interactable2, interactable3, warmupInteractable];
};
...
const addMainText = async (
  ...
  const borderTextMesh = new THREE.Mesh(
    outerGeometry,
    new THREE.MeshBasicMaterial({ color: 0x253363 }),
  );
  ...
  const titleBorderTextMesh = new THREE.Mesh(
    titleOuterGeometry,
    new THREE.MeshBasicMaterial({ color: 0x253363 }),
  );
...
const addSkills = (
  ...
  const skills: SkillTransform[] = [
    ...
    {
      name: "laravel",
      position: new THREE.Vector3(-8.2, 1.9, -0.5),
      rotation: new THREE.Euler(Math.PI / 2, 0, -Math.PI / 2),
      scale: new THREE.Vector3(0.1, 0.1, 0.1),
    },
    {
    ...
    {
      name: "nestjs",
      position: new THREE.Vector3(-8.2, 1.9, -2.2),
      rotation: new THREE.Euler(Math.PI / 2, 0, -Math.PI / 2),
      scale: new THREE.Vector3(0.1, 0.1, 0.1),
    },
  ];
  ...
  skills.forEach(async (skill) => {
    const model = await loadGLTFModel(
      `assets/logos/skills/${skill.name}.glb`,
      loader,
    );
