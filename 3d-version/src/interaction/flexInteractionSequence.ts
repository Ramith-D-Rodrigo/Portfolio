...
class FlexInteractionSequence extends BaseInteractionSequence {
  public override async playSequence(
    ...
    for (let i = 0; i < animations.length; i++) {
      await this.playAnimation(animations[i], animationMap, mixer);
    }
  ...
  protected override playAnimation(
    animProps: InteractionAnimProps,
    animationMap: Map<string, THREE.AnimationAction>,
    mixer: THREE.AnimationMixer,
  ): Promise<void> {
    return new Promise((resolve) => {
      ...
      const loopFunc = () => {
        const { title, description, list } =
          animProps.displayTextList[displayIdx++];
        if (title === "") return;
        ...
        const formattedText = `
                ...
                `;
        InteractionDescHUD.getInstance().setDisplayText(
          formattedText,
          animProps.displayTextDur,
        );
      };
      loopFunc();
