const degreeInfo = {
  general: {
    degree: "Bachelor of Science Honours in Computer Science",
    university: "University of Colombo School of Computing",
    graduationYear: 2025,
    gpa: "3.88/4.00",
    period: "April 2021 - June 2025",
    summary:
      "After excelling in the G.C.E. Advanced Level examination, I was selected to pursue a Bachelor of Science in Computer Science at the University of Colombo School of Computing. " +
      "My dedication and passion for the field enabled me to excel during my first two academic years, qualifying me for the prestigious Honours degree program. " +
      "During my final year, I conducted research focused on enhancing the performance of web-based extended reality applications. " +
      "Throughout my academic journey, I have consistently taken on leadership roles in group projects and assignments, ensuring effective collaboration " +
      "among team members to deliver efficient and timely solutions.",
    link: "https://ucsc.cmb.ac.lk/",
  },
  coreCourses: [
    "Data Structures and Algorithms",
    "Computer Graphics",
    "Operating Systems",
    "Game Development",
    "Software Engineering",
    "Middleware Architecture",
    "Compiler Theory",
    "Database I & II",
    "Object-Oriented Programming",
    "Machine Learning and Neural Computing",
    "Image Processing & Computer Vision",
    "Parallel Computing",
  ],
  achievements: [
    {
      title: "Director's List - 2nd Academic Year",
      image: "/education/achievements/2nd_year.jpg",
    },
    {
      title: "Director's List - 3rd Academic Year 1st Semester",
      image: "/education/achievements/3rd_year.jpg",
    },
  ],
};

export { degreeInfo };
